# 知識ベース1: 技術仕様

## 📦 技術スタック

### Backend
```
Django==5.2.6          # LTS版（2028年4月サポート）
wagtail==7.0.6        # Django 5.2対応
djangorestframework==3.16.1
django-import-export==4.2.0
django-redis==5.4.0
psycopg2-binary==2.9.9
APScheduler==3.10.4    # タスクスケジューラー
```

### Frontend
```
next==16.0.0          # Turbopack標準搭載
react==19.1.0
typescript==5.x
@tanstack/react-query==5.59.0
shadcn/ui
tailwindcss==3.x
next-devtools-mcp==latest  # MCP統合
```

### Infrastructure
```
PostgreSQL 14+（13はサポート終了）
Redis 5.1.0+
Docker Compose
```

## 🏗 アーキテクチャ構成

### フロントエンド（ユーザー向け）
- **Next.js 16.0.0**（App Router）
  - Turbopack: デフォルト有効（ビルド2.6倍高速）
  - React 19サポート
  - Cache Components対応
- **役割**: ユーザーが実際に見る画面
- **ルート例**: `/tools/`, `/blog/`, `/ranking/`

### バックエンド（API + CMS）
- **Django REST API**: データ提供（JSON）
- **Wagtail CMS**: Headless構成（コンテンツ管理）
- **Django Templates**: **管理画面のみ**
  - `/admin/`（Django Admin）
  - `/cms/`（Wagtail Admin）
  - ユーザー向け画面には使用しない

### データフロー
```
[ユーザー]
    ↓
[Next.js 16 (フロントエンド)]
    ↓ HTTP Request
[Django REST API]
    ↓ Query
[PostgreSQL / Redis]
```

## 🔧 開発ツール

### Next.js DevTools（MCP統合）
- **next-devtools-mcp**: Next.js開発サーバー情報をClaude Desktopに提供
- **機能**:
  - リアルタイムビルドエラー検出
  - ハイドレーションエラー検出
  - ルート情報自動取得
  - 型エラー検出
  - 環境変数確認

### Turbopack（高速ビルド）
- **Next.js 16でデフォルト有効**
- **パフォーマンス**:
  - ビルド速度: Webpack比 2.6倍高速
  - HMR: 40-50%高速化
  - メモリ使用量削減
- **使用方法**: 自動有効（設定不要）

## 📊 データモデル

### Tool（ツール）
```python
class Tool(models.Model):
    # 識別子
    id = models.UUIDField(primary_key=True, default=uuid.uuid4)
    slug = models.SlugField(unique=True, max_length=200)

    # 基本情報
    name = models.CharField(max_length=200)
    short_description = models.TextField(max_length=100)
    long_description = models.TextField()

    # 分類
    platform = ArrayField(models.CharField(max_length=20))
    # 値: ['mt4'], ['mt5'], ['tradingview'], ['mt4', 'mt5']

    tool_type = models.CharField(max_length=20, choices=[
        ('EA', 'Expert Advisor'),
        ('Indicator', 'インジケーター'),
        ('Library', 'ライブラリ'),
        ('Script', 'スクリプト'),
        ('Strategy', 'ストラテジー'),
    ])

    price_type = models.CharField(max_length=20, choices=[
        ('free', '無料'),
        ('paid', '有料'),
        ('freemium', 'フリーミアム'),
    ])

    # メタ情報
    image_url = models.URLField()
    external_url = models.URLField()
    metadata = models.JSONField(default=dict)

    # タイムスタンプ
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
```

### BlogPage（Wagtailページ）
```python
from wagtail.models import Page
from wagtail.fields import StreamField

class BlogPage(Page):
    # メタ情報
    excerpt = models.TextField(max_length=200)
    category = models.CharField(max_length=30)
    investment_type = models.CharField(max_length=20)

    # コンテンツ（StreamField）
    body = StreamField([...])  # 詳細は知識ベース2参照

    # 関連
    featured_image = models.ForeignKey('wagtailimages.Image')
    related_tools = models.ManyToManyField(Tool, blank=True)

    # 統計
    view_count = models.IntegerField(default=0, editable=False)
```

### ToolStats（統計）
```python
class ToolStats(models.Model):
    tool = models.OneToOneField(Tool, on_delete=models.CASCADE)

    # 週間統計
    week_views = models.IntegerField(default=0)
    week_shares = models.IntegerField(default=0)
    week_cta_clicks = models.IntegerField(default=0)
    week_avg_duration = models.FloatField(default=0)
    week_score = models.FloatField(default=0)
    prev_week_rank = models.IntegerField(null=True)

    # 更新日時
    last_updated = models.DateTimeField(auto_now=True)
```

### EventLog（イベント記録）
```python
class EventLog(models.Model):
    EVENT_TYPES = [
        ('view', 'ページ閲覧'),
        ('share', 'SNSシェア'),
        ('duration', '滞在時間'),
        ('cta_click', 'CTAクリック'),
    ]

    tool = models.ForeignKey(Tool, on_delete=models.CASCADE)
    event_type = models.CharField(max_length=20, choices=EVENT_TYPES)
    created_at = models.DateTimeField(auto_now_add=True)

    # オプションフィールド
    duration_seconds = models.IntegerField(null=True)
    share_platform = models.CharField(max_length=20, null=True)
    cta_type = models.CharField(max_length=20, null=True)  # download, purchase, external
```

## 🚀 API仕様

### エンドポイント一覧

#### 1. ツール検索
```
GET /api/tools/

パラメータ:
- platform: mt4|mt5|tradingview
- tool_type: EA|Indicator|Library|Script|Strategy
- price_type: free|paid|freemium
- q: フリーワード検索
- sort: -week_score|created_at|price
- page: ページ番号

レスポンス例:
{
  "results": [
    {
      "id": "uuid",
      "name": "Grid Master EA",
      "slug": "grid-master-ea",
      "platform": ["mt5"],
      "week_rank": 1,
      "week_score": 157.5
    }
  ],
  "count": 150
}
```

#### 2. ツール詳細
```
GET /api/tools/{slug}/

レスポンス:
{
  "id": "uuid",
  "name": "Grid Master EA",
  "long_description": "詳細説明...",
  "stats": {
    "week_views": 250,
    "week_score": 157.5,
    "week_rank": 1,
    "rank_change": "↑3"
  }
}
```

#### 3. 週間ランキング
```
GET /api/ranking/weekly/

パラメータ:
- limit: 取得件数（デフォルト20、最大100）
- platform: mt4|mt5|tradingview

レスポンス:
{
  "updated_at": "2025-01-20T02:00:00Z",
  "rankings": [
    {
      "rank": 1,
      "rank_change": "NEW",
      "tool": {...},
      "score": 157.5
    }
  ]
}
```

#### 4. イベントトラッキング
```
POST /api/events/track/

リクエスト:
{
  "target_type": "tool",
  "target_id": "uuid",
  "event_type": "view|share|duration|cta_click",
  "duration_seconds": 120,
  "cta_type": "download|purchase|external"
}

レスポンス: 204 No Content
```

## 🗂 プロジェクト構造

```
toolradar/
├── backend/
│   ├── config/
│   │   ├── settings/
│   │   │   ├── base.py
│   │   │   ├── local.py
│   │   │   └── production.py
│   │   └── urls.py
│   │
│   ├── apps/
│   │   ├── tools/
│   │   │   ├── models.py
│   │   │   ├── serializers.py
│   │   │   ├── views.py
│   │   │   ├── scheduler.py  # APScheduler
│   │   │   └── management/commands/
│   │   │
│   │   ├── blog/
│   │   │   ├── models.py
│   │   │   ├── blocks.py  # StreamField
│   │   │   └── wagtail_hooks.py
│   │   │
│   │   └── analytics/
│   │       └── tasks.py  # ランキング集計
│   │
│   └── logs/  # APSchedulerログ
│
├── frontend/
│   ├── app/
│   │   ├── layout.tsx
│   │   ├── page.tsx
│   │   ├── tools/
│   │   │   └── [slug]/
│   │   │       └── page.tsx
│   │   └── ranking/
│   │       └── weekly/
│   │           └── page.tsx
│   │
│   ├── components/
│   │   ├── ui/          # shadcn/ui
│   │   └── features/
│   │
│   └── lib/
│       ├── api.ts
│       └── utils.ts
│
└── docker-compose.yml
```

## 🔐 認証・権限

### API認証
- 読み取りAPI: 認証不要（公開）
- 書き込みAPI: Django Admin認証必須
- イベントトラッキング: CORS制限のみ

### 管理画面
- Django Admin: `/admin/`
- Wagtail Admin: `/cms/`
- スーパーユーザーのみアクセス可

### CORS設定
```python
CORS_ALLOWED_ORIGINS = [
    "http://localhost:3000",  # 開発
    "https://toolradar.jp",  # 本番
]
```
