...
# 週間スコア計算（意図の強さで重み付け）
WeeklyScore = (
    (週間CTAクリック × 5.0) +       # 最重要: 購買/DL意図
    (週間シェア数 × 2.0) +          # 重要: 推薦意図
    (平均滞在時間(秒) ÷ 10 × 0.5) + # 中程度: エンゲージメント
    (週間PV数 × 0.3)                # ベース: 認知
)

# 計算例
CTAクリック: 10, シェア: 5, 滞在: 120秒, PV: 200
= (10 × 5.0) + (5 × 2.0) + (120 ÷ 10 × 0.5) + (200 × 0.3)
= 50 + 10 + 6 + 60
= 126ポイント
```
...
| 指標 | 重み | 理由 |
|------|------|------|
| **CTAクリック** | ×5.0 | 「このツールを使いたい」という最強シグナル |
| **シェア** | ×2.0 | 「他人にも勧めたい」という高評価シグナル |
| **滞在時間** | ÷10 × 0.5 | 「じっくり検討している」エンゲージメント |
| **PV** | ×0.3 | 「興味がある」という最小シグナル |

### ランキング表示構成

| 場所 | 表示数 | 目的 |
|------|--------|------|
| トップページ | TOP3 | UX・注目度重視 |
| ランキング専用ページ | TOP20〜全件 | SEO・広告収益 |
| ブログ記事内 | 関連3〜5件 | 文脈に沿った紹介 |

※ TOP3で全クリックの約50%を占める（7位以下はCTR 0.5%以下）
...
# cta_click（CTAクリック）
- 記録: tool_id, cta_type, timestamp
- 対象: download, purchase, external
- 重複除外: 同一セッションは1回
```
...
| 目的 | CTA文言 | 補足テキスト |
|------|---------|-------------|
| 口座開設 | 無料で口座開設 | 最短5分で申込完了 |
| キャンペーン | 特典を受け取る | 今なら○○円キャッシュバック |
| 比較検討 | 詳細を確認する | スプレッド・手数料を比較 |
